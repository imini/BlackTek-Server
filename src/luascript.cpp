...
int LuaScriptInterface::luaPlayerGetDepotItemCount(lua_State* L)
{
	...
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		lua_pushnumber(L, player->getDepotItemCount());
	}
	else {
		lua_pushnil(L);
	}
...
int LuaScriptInterface::luaActionItemId(lua_State* L)
{
	...
	if (action) {
		...
		if (parameters > 1) {
			for (int i = 0; i < parameters; ++i) {
				g_actions->addItemId(action, getNumber<uint16_t>(L, 2 + i));
			}
		} else {
			g_actions->addItemId(action, getNumber<uint16_t>(L, 2));
		}
		pushBoolean(L, true);
	} else {
		lua_pushnil(L);
...
int LuaScriptInterface::luaActionActionId(lua_State* L)
{
	...
	if (action) {
		...
		if (parameters > 1) {
			for (int i = 0; i < parameters; ++i) {
				g_actions->addActionId(action, getNumber<uint16_t>(L, 2 + i));
			}
		} else {
			g_actions->addActionId(action, getNumber<uint16_t>(L, 2));
		}
		pushBoolean(L, true);
	} else {
		lua_pushnil(L);
...
int LuaScriptInterface::luaActionUniqueId(lua_State* L)
{
	...
	if (action) {
		...
		if (parameters > 1) {
			for (int i = 0; i < parameters; ++i) {
				g_actions->addUniqueId(action, getNumber<uint16_t>(L, 2 + i));
			}
		} else {
			g_actions->addUniqueId(action, getNumber<uint16_t>(L, 2));
		}
		pushBoolean(L, true);
	} else {
		lua_pushnil(L);
...
int LuaScriptInterface::luaMoveEventPosition(lua_State* L)
{
	...
	if (moveevent) {
		...
		if (parameters > 1) {
			for (int i = 0; i < parameters; ++i) {
				g_moveEvents->addPosList(moveevent, getPosition(L, 2 + i));
			}
		} else {
			g_moveEvents->addPosList(moveevent, getPosition(L, 2));
		}
		pushBoolean(L, true);
	} else {
		lua_pushnil(L);
